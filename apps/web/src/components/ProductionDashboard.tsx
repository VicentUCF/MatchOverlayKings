...
export function ProductionDashboardView({
  ...
  const configuredCount = enabledCourts.filter(({ slug }) => configurationFor(configurations, slug) !== null).length;
  const liveCount = enabledCourts.filter(({ slug }) => {
  ...
  const content = <>
    <section className="production-dashboard" aria-labelledby="production-dashboard-title">
      <header className="production-dashboard-heading">
        <div><p className="production-kicker">Jornada en directo</p><h1 id="production-dashboard-title">Todas las pistas, en un solo sitio</h1>
        ...
        <dl className="production-dashboard-summary">
          <div><dt>Configuradas</dt><dd>{configuredCount}/{enabledCourts.length}</dd></div>
          <div><dt>Emitiendo</dt><dd>{liveCount}/{enabledCourts.length}</dd></div>
          <div><dt>Runtime</dt><dd>{state.kind === 'ready' ? 'Conectado' : 'Sin conexión'}</dd></div>
        </dl>
      </header>
...
function UnifiedCourtCard({ court, configuration, session, loading, onOpenConfiguration, onOpenControls }: {
  ...
  const status = streamStatus(session, loading, court.productionEnabled);
  ...
  return <article className="production-dashboard-court" aria-labelledby={`dashboard-${court.slug}`}>
    <header><div><span className="production-court-card__slug">{court.slug}</span><h2 id={`dashboard-${court.slug}`}>{court.name}</h2></div>
    ...
    <div className="production-dashboard-preview">
      <iframe src={`/overlay/${court.slug}/scoreboard`} title={`Vista previa del overlay de ${court.name}`} tabIndex={-1} />
      <span><MonitorPlay aria-hidden="true" />Vista previa del programa</span>
    </div>
    <div className="production-dashboard-court__body">
      {!court.productionEnabled ? <div className="production-dashboard-empty"><strong>Producción desactivada</strong>
        <p>Esta pista está visible porque forma parte del inventario, pero no admite emisiones.</p></div>
      ...
      <nav className="production-dashboard-court__actions" aria-label={`Controles de ${court.name}`}>
        {configuration === null
...
function streamStatus(session: PilotSession | null, loading: boolean, enabled: boolean): { readonly label: string; readonly tone: string } {
