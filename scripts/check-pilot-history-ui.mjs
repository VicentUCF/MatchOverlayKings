...
try {
  ...
  assert.equal(await page.getByRole('button', { name: 'Emitir', exact: true }).isDisabled(), true, 'preflight required before broadcast');
  ...
  assert.equal(await page.getByRole('button', { name: 'Emitir', exact: true }).isDisabled(), true);
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false, 'preflight 320px reflow');
  await page.screenshot({ path: `${root}/output/mejoras/preflight-blocked-320.png`, fullPage: true });
  ...
  await page.waitForFunction(() => ![...document.querySelectorAll('button')].find((button) => button.textContent === 'Emitir')?.disabled);
  assert.equal((await page.evaluate(() => window.fixtureActions)).at(-1), 'check:storage');
  const retryNetwork = page.getByRole('button', { name: 'Repetir: Red y capacidad de subida', exact: true });
  if (!await retryNetwork.isVisible()) await page.getByText('Resultados de las comprobaciones', { exact: true }).click();
  ...
  assert.equal(await page.getByRole('button', { name: 'Emitir', exact: true }).isDisabled(), true, 'stale checks block broadcast');
  console.log('PASS: required preflight, cancellation, failed-check and fresh-network keyboard retry, upload notice, stale guard and 320px reflow');
  await page.setViewportSize({ width: 320, height: 900 });
